# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
# Functions for validating the seq2seq model
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
import sys
sys.path.append("../src")
from data_processing.cornelMovie_preProcessing import normalizeString
from trainers.auxilary_for_trainers import indexesFromSentence
...
class GreedySearchDecoder(nn.Module):
    ...
    def forward(self, input_seq, input_length, max_length):
        ### Maybe this shouldn't be here, but for now this is fix
